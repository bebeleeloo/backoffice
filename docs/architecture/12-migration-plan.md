...
```
Frontend (3 SPA) ─── nginx ───── API Gateway (:8090) ───┬── Monolith API (:8080) ──── PostgreSQL (:5432)
     :3000                                              └── Auth Service (:8082) ──┘      public.* / auth.*
                                                             n8n (:5678) ── n8n-db
```

- 2 бэкенд-сервиса (monolith + auth)
...
┌─────────────┐             ┌───────────┐       ┌─────────────┐
│ backoffice  │──REST──────▶│  Gateway  │──┬───▶│  Monolith   │ gRPC/REST
...
3. **REST Proxy Middleware**
   - Маршрутизация `GET /api/v1/clients` → `monolith`
...
- [x] REST proxy к monolith и auth service
