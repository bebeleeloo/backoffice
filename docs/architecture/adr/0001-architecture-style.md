...
Аутентификация и управление пользователями/ролями/правами вынесены в отдельный **auth-service** (микросервис). Монолит сохраняет бизнес-логику (клиенты, счета, инструменты, ордера, транзакции, аудит). Оба сервиса следуют Clean Architecture. Монолит валидирует JWT локально (без roundtrip к auth-service) и обращается к auth-service только для получения статистики пользователей (dashboard) через `IAuthServiceClient`.
