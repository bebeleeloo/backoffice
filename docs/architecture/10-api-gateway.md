...
API Gateway -- центральный сервис-прослойка между внешними потребителями (Frontend, n8n, внешние интеграции) и внутренними сервисами (Core, Auth Service, будущие сервисы). Объединяет три ключевые функции:
...
sequenceDiagram
    ...
    participant M as Core (gRPC/REST)
...
Frontend → nginx → Gateway (REST) → core (gRPC или REST)
...
```
n8n → core (REST, http://api:8080/api/v1)
...
### Phase 2: gRPC-сервисы в Core и Auth
...
**Задачи:**
1. Перевести core upstream на `protocol: grpc` в YAML (list queries — основная нагрузка)
