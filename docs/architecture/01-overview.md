...
```
new-back/
  docker-compose.yml          # Оркестрация всех сервисов (mssql, auth, api, web)
...
- **Монолит (backend)** -- JWT-валидация (локальная проверка claims, без обращения к auth-service), бизнес-операции. Использует схему `dbo.*` в БД. Вызывает auth-service через `IAuthServiceClient` только для получения статистики пользователей (dashboard).
