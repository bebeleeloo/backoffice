...
public class UpdateUserValidatorTests
{
    ...
    public void ValidCommand_ShouldPass()
    {
        var result = _validator.TestValidate(new UpdateUserCommand(
            Guid.NewGuid(), "user@test.com", "Full Name", true, [], [1, 2, 3]));
    ...
    {
        var result = _validator.TestValidate(new UpdateUserCommand(
            Guid.NewGuid(), email!, "Name", true, [], [1, 2, 3]));
    ...
    {
        var result = _validator.TestValidate(new UpdateUserCommand(
            Guid.NewGuid(), "not-an-email", "Name", true, [], [1, 2, 3]));
        result.ShouldHaveValidationErrorFor(x => x.Email);
    }
    ...
    {
        var result = _validator.TestValidate(new UpdateUserCommand(
            Guid.NewGuid(), new string('a', 248) + "@test.com", "Name", true, [], [1, 2, 3]));
        result.ShouldHaveValidationErrorFor(x => x.Email);
    }
    ...
    {
        var result = _validator.TestValidate(new UpdateUserCommand(
            Guid.NewGuid(), "user@test.com", "Name", true, [], Array.Empty<byte>()));
